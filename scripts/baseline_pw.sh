...
OUT_DIRNAME=baseline_pw${pos_weight}$_"$residues"_"$type"_"$suffix"
...
if compgen -G "new_logs/$OUT_DIRNAME/fold_0/*.ckpt" > /dev/null; then
	...
	python3 baseline.py --epochs=30 --batch_size=4 --accum=8 --pos_weight="$pos_weight" --prot_info_path "$prot_info" --dataset_path "$splits" --lr=5e-4 -o "$OUT_DIRNAME" --num_workers=15 --residues="$residues" --modify_prob=0 --type="$type" --fold=0
fi
